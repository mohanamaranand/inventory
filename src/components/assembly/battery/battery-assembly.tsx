...
import type { BatteryModel } from "@/lib/types";
import { CreateBatteryModelForm } from "./create-battery-model-form";
import { AssembleBattery } from "./assemble-battery";
import { AssembledBatteriesTable } from "./assembled-batteries-table";


export function BatteryAssembly() {
  const [isSheetOpen, setSheetOpen] = useState(false);
  const [selectedModelId, setSelectedModelId] = useState<string | null>(null);
  const { batteryModels, addBatteryModel, getItemByStdCode } = useInventory();
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const modelsToCreate = new Map<string, Omit<BatteryModel, "id">>();
        ...
        modelsToCreate.forEach((model) => {
          addBatteryModel(model);
        });

        toast({
          title: "Import Complete",
          description: `${modelsToCreate.size} new battery models have been added.`,
        });
    ...
    <>
      ...
      <Tabs defaultValue="assemble">
        <TabsList className="grid w-full grid-cols-3">
          <TabsTrigger value="assemble">Assemble Battery</TabsTrigger>
          <TabsTrigger value="models">Battery Models</TabsTrigger>
          <TabsTrigger value="assembled">Assembled Batteries</TabsTrigger>
        </TabsList>
          ...
          <Card>
            <CardHeader>
              <div className="flex justify-between items-center">
                <div>
                  <CardTitle>Battery Models</CardTitle>
                  <CardDescription>
                    Browse and manage your saved battery models.
                  </CardDescription>
                </div>
                <div>
                  <input
